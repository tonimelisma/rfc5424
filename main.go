...
func parsePriority(priorityNr int) (facility string, severity string) {
	severityNr := priorityNr % 8
	facilityNr := (priorityNr - severityNr) / 8
	severity, err := parseSeverity(severityNr)
	if err != nil {
		severity = "undefined"
	}
	facility, err = parseFacility(facilityNr)
	if err != nil {
		severity = "undefined"
	}
...
// parseFacility returns a string describing the facility of the log message
func parseFacility(facilityNr int) (facility string, err error) {
	if facilityNr < 0 || facilityNr >= len(facilities) {
		return "", fmt.Errorf("facility out of range [%v]", facilityNr)
	}
	return facilities[facilityNr], nil
}

// parseSeverity returns a string describing the severity of the log message
func parseSeverity(severityNr int) (severity string, err error) {
	if severityNr < 0 || severityNr >= len(severities) {
		return "", fmt.Errorf("severity out of range [%v]", severityNr)
	}
	return severities[severityNr], nil
}
